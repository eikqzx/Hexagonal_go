...
func (r *GetSVACadastralImageNullImpl) Get02CadastralImage(cadastralSeq int64) ([]model.CadastralImage, error) {
	if DB == nil {
		InitDB()
	}
	queryPath := filepath.Join("sql", "dataQuery", "02getCadastralImage.sql")
	query, err := ReadSQLFromFile(queryPath)
	if err != nil {
		return nil, fmt.Errorf("failed to read SQL query file: %v", err)
	}
