...
func (r *LandOfficeRepositoryImpl) FetchAllLandOffice() ([]model.LandOffice, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&landOffice.TAX_ID, &landOffice.LANDOFFICE_NOTE,
			&landOffice.IPV4_ADDRESS,
...
func (r *LandOfficeRepositoryImpl) FetchSVALandOffice() ([]model.LandOfficeSVA, error) {
	if DB == nil {
		InitDB()
	}
	queryPath := filepath.Join("sql", "MAS", "TB_MAS_LANDOFFICE", "selectFromSVA.sql")
	// log.Println(queryPath)
	query, err := ReadSQLFromFile(queryPath)
	if err != nil {
		return nil, fmt.Errorf("failed to read SQL query file: %v", err)
	...
	var landOffices []model.LandOfficeSVA
	for rows.Next() {
		var landOffice model.LandOfficeSVA
		if err := rows.Scan(
			&landOffice.LANDOFFICE_SEQ, &landOffice.LANDOFFICE_SYS_, &landOffice.LANDOFFICE_NO,
			&landOffice.LANDOFFICE_ID, &landOffice.LANDOFFICE_ABBR, &landOffice.LANDOFFICE_NAME_TH,
			&landOffice.LANDOFFICE_NAME_EN, &landOffice.PROVINCE_SEQ, &landOffice.PC_SERVER,
			&landOffice.CADASTRAL_SEQ, &landOffice.CADASTRAL_IMAGE_SEQ, &landOffice.IP_ADDRESS,
			&landOffice.LANDOFFICE_NOTE, &landOffice.RECORD_STATUS, &landOffice.CREATE_USER,
			&landOffice.CREATE_DTM, &landOffice.LAST_UPD_USER, &landOffice.LAST_UPD_DTM,
		); err != nil {
			return nil, fmt.Errorf("failed to scan row: %v", err)
		}
		landOffices = append(landOffices, landOffice)
...
func (r *LandOfficeRepositoryImpl) FetchLandOfficeByProvinceSeq(provinceSeq int64) ([]model.LandOffice, error) {
	if DB == nil {
		InitDB()
	}
	queryPath := filepath.Join("sql", "MAS", "TB_MAS_LANDOFFICE", "landoffice.sql")
	query, err := ReadSQLFromFile(queryPath)
	if err != nil {
		return nil, fmt.Errorf("failed to read SQL query file: %v", err)
	}

	rows, err := DB.Query(query, provinceSeq)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&landOffice.TB_TOR_CADASTRAL, &landOffice.TB_TOR_CADASTRAL_LAND, &landOffice.TB_TOR_CADASTRAL_OWNER,
			&landOffice.TB_TOR_CADASTRAL_IMAGE, &landOffice.TB_TOR_CADASTRAL_BORROW,
			&landOffice.TB_TOR_CIRACORE_IMAGE, &landOffice.PROVINCE_NAME_TH,
		); err != nil {
			return nil, fmt.Errorf("failed to scan row: %v", err)
		}
		landOffices = append(landOffices, landOffice)
