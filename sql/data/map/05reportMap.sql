SELECT * FROM (
    SELECT TMAP.OGR_FID , TMAP.UTMMAP1, TMAP.UTMMAP2, TMAP.UTMMAP3, TMAP.UTMMAP4 , TMAP.UTMSCALE , tMap.LAND_NO , tLand.LAND_NO AS LAND_LINK, 
    tParcel.LAND_NO AS PARCEL_LINK,
    ...
    LEFT JOIN (
        SELECT *
        FROM (
        	SELECT t.UTMMAP1, t.UTMMAP2, t.UTMMAP3, t.UTMMAP4, t.LAND_NO , tScale.SCALE_NAME_TH AS UTMSCALE FROM (
            SELECT
              PARCEL_UTMMAP1 AS UTMMAP1  ,
              PARCEL_UTMMAP2 AS UTMMAP2 , 
              PARCEL_UTMMAP3 AS UTMMAP3 ,
              PARCEL_UTMMAP4 AS UTMMAP4 ,
             UTMSCALE_SEQ  ,
             PARCEL_UTM_LAND_NO AS LAND_NO
            FROM REG.TB_REG_PARCEL trp
            WHERE RECORD_STATUS = 'N'
          )t LEFT JOIN MAS_.TB_MAS_SCALE tScale
          ON UTMSCALE_SEQ = SCALE_SEQ
        ) t
        GROUP BY t.UTMMAP1, t.UTMMAP2, t.UTMMAP3, t.UTMMAP4, t.UTMSCALE, t.LAND_NO
    ) tParcel
    ON TMAP.UTMMAP1 = tParcel.UTMMAP1
    AND TMAP.UTMMAP2 = tParcel.UTMMAP2
    AND TMAP.UTMMAP3 = tParcel.UTMMAP3
    AND TMAP.UTMMAP4 = tParcel.UTMMAP4
    AND TMAP.UTMSCALE = tParcel.UTMSCALE
    AND TMAP.LAND_NO = tParcel.LAND_NO
...
WHERE rn >=(NVL(:PageNumber,1)-1 ) * 100 + 1 
AND rn <= NVL(:PageNumber,1) * 100
